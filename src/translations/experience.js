const experience = {
  English: {
    experience: 'EXPERIENCE',
    photos: 'PHOTOS',
    videos: 'VIDEOS',
    videoSubtitle: 'Find out what our affiliated professors, students and parents have to say about their experience with Concord.',
    video_moana: 'Moana Jiao, Anna Jiao (students of Concord Music)',
    video_agustin: 'Dr. Agustin Muriago (Piano Professor of Kansas State University, USA)',
    video_agnes: 'Agnes Graglia (parent of Concord Music)',
    video_oching: 'O Ching (student of Concord Music)',
    video_nancy: 'Nancy (parent of Concord Music)',
    video_grace: 'Grace (parent of Concord Music)',
    video_linus: 'Linus (student of Concord Music)',
    video_david: 'David (student of Concord Music)',
    exsubtitle: 'EXPERIENCE',
    extitle: 'What is learning like at Concord Music?',
    excontent: 'Many say that they can immediately feel the warmth when they first visit Concord; they can see that our students really enjoy their lessons and our teachers are warm, friendly and professional. We keep our passion going and we know that this will inspire our students to keep theirs going too! Concord has been growing through word-of-mouth and recommendation from one to another and we continue to strive to gain more trust through actual effort. So it is important for you to hear what others say about us too.'
  },
  Chinese: {
    experience: '体验搿艺',
    photos: '图片',
    videos: '视频',
    videoSubtitle: '来听听著名教授们，搿艺的学员家⻓们分享他们与搿艺音乐的故事...',
    video_moana: '焦曼婷  (搿艺音乐的学员)',
    video_agustin: 'Agustin Muriago博士（美国堪萨斯州立大学的钢琴教授）',
    video_agnes: 'Agnes Graglia (搿艺音乐学员的母亲）',
    video_oching: 'O Ching (搿艺音乐的学员)',
    video_nancy: 'Nancy (搿艺音乐学员的母亲)',
    video_grace: 'Grace (搿艺音乐学员的母亲)',
    video_linus: 'Linus (搿艺音乐的学员)',
    video_david: 'David (搿艺音乐的学员)',
    exsubtitle: '体验搿艺',
    extitle: '在搿艺学习感受如何？',
    excontent: '很多认识搿艺的人说，当他们第一次来访搿艺就感觉特别温暖，因为他们看到学生们在琴房里上课很享受的，老师亲切友善而且很专业。我们继续保持我们的热情，因为我们知道这会正面地影响我们学员们的热情和坚持！搿艺自成立，靠的是口碑，我们获得的信任也是靠努力累积的。所以，除了听我们说，请聆听其他人如何反馈有关我们。'
  }
}
export default experience
